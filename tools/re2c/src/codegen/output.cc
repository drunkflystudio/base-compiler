...
Ret Output::new_block(Opt& opts, InputBlock kind, std::string name, const loc_t& loc) {
    ...
    if (kind == InputBlock::USE) {
        // `use` blocks have autogenerated names that differ from the name of the `rules` block that
        // they are using, so the user cannot reference a `use` block in any way. (They can easily
        // achieve the same goal with the help of the `!use` directive in a normal block.)
